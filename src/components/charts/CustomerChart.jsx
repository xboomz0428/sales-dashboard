        ...
        <div className="flex gap-1 bg-gray-100 dark:bg-gray-700/60 p-1 rounded-lg">
          {[{ v: 'ranking', l: '客戶排行' }, { v: 'channel', l: '通路客群' }].map(t => (
            <button key={t.v} onClick={() => setSubTab(t.v)}
              className={`px-2 sm:px-3 py-1.5 min-h-[36px] rounded-md text-sm sm:text-base font-medium transition-all ${subTab === t.v ? 'bg-white dark:bg-gray-800 text-blue-700 dark:text-blue-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
              {t.l}
            </button>
          ))}
        </div>
