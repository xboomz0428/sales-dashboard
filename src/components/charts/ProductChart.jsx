          ...
          <div className="flex gap-1 bg-gray-100 dark:bg-gray-700/60 p-1 rounded-xl">
            {tabs.map(t => (
              <button key={t.v} onClick={() => setSubTab(t.v)}
                className={`px-3 py-1.5 rounded-lg text-base font-medium transition-all ${subTab === t.v ? 'bg-white dark:bg-gray-800 text-blue-700 dark:text-blue-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
                {t.l}
              </button>
            ))}
          </div>
