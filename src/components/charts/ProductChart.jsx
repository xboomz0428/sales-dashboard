    ...
    <div className="bg-white border border-gray-200 rounded-2xl shadow-xl p-4 text-sm min-w-[200px]">
      ...
      {payload.map((e, i) => (
        <div key={i} className="flex justify-between gap-4 py-0.5">
          ...
          <span className="font-mono font-bold text-gray-800">{e.value?.toLocaleString()}</span>
        </div>
      ))}
    </div>
  )
...
function RankingChart({ data, metric }) {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h4 className="text-base font-bold text-gray-700 mb-4">前8大產品佔比</h4>
        <ResponsiveContainer width="100%" height={280}>
          <PieChart>
            ...
            <Legend layout="vertical" align="right" verticalAlign="middle" wrapperStyle={{ fontSize: 13 }}
              formatter={(v, e) => <span style={{ color: '#374151' }}>{v} ({fmtY(e.payload[metric])})</span>} />
          </PieChart>
        </ResponsiveContainer>
        {/* Summary table */}
...
function ChannelChart({ productByChannel, metric }) {
  ...
  if (!data.length) return <div className="flex items-center justify-center h-64 text-gray-400 text-base">無資料</div>
  const maxNameLen = Math.max(0, ...data.map(d => d.product?.length || 0))
  const yAxisWidth = Math.min(220, Math.max(120, maxNameLen * 8))
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      <h4 className="text-base font-bold text-gray-700 mb-4">產品 × 通路銷售分布（前15產品，堆疊）</h4>
...
function CustomerDistChart({ productData, productCustomerData, metric }) {
  ...
  if (!top8.length) return <div className="flex items-center justify-center h-64 text-gray-400 text-base">無資料</div>
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
      {top8.map((prod, pi) => {
        const customers = productCustomerData[prod.name] || []
        return (
          <div key={prod.name} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-gray-400">{fmtY(prod[metric])}</span>
            </div>
            {customers.length === 0 ? (
              <p className="text-sm text-gray-400">無客戶資料</p>
            ) : (
                  ...
                  return (
                    <div key={c.name}>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="font-mono text-gray-700">{fmtY(c[metric])}</span>
                      </div>
    ...
    <ChartCard title={`產品分析 — ${label}`}>
      <div className="space-y-5">
          ...
          <div className="flex gap-1 bg-gray-100 p-1 rounded-xl">
            {tabs.map(t => (
              <button key={t.v} onClick={() => setSubTab(t.v)}
                className={`px-3 py-1.5 rounded-lg text-base font-medium transition-all ${subTab === t.v ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                {t.l}
              </button>
            ))}
          </div>
        ...
        {productData.length === 0 ? (
          <div className="flex items-center justify-center h-64 text-base text-gray-400 bg-gray-50 rounded-2xl border">
