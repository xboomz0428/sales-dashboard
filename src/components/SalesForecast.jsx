  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">🔮 銷售預測分析</h2>
          <p className="text-base text-gray-400 dark:text-gray-500 mt-0.5">基於線性回歸 + 季節性調整，預測未來銷售走勢</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-base text-gray-500 dark:text-gray-400">預測月數</span>
          <select value={months} onChange={e => setMonths(Number(e.target.value))}
            className="px-3 py-1.5 border border-gray-200 dark:border-gray-600 rounded-xl text-base focus:outline-none focus:border-blue-400 bg-white dark:bg-gray-700 dark:text-gray-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-700 rounded-2xl p-4">
          <p className="text-sm text-blue-600 dark:text-blue-400 font-semibold">模型準確度 R²</p>
          <p className="text-2xl font-black text-blue-700 dark:text-blue-300 mt-1">{r2 != null ? (r2 * 100).toFixed(0) + '%' : '—'}</p>
          <p className="text-xs text-blue-400 dark:text-blue-500 mt-0.5">{r2 >= 0.8 ? '高擬合' : r2 >= 0.5 ? '中擬合' : '低擬合'}</p>
        </div>
        <div className="bg-purple-50 dark:bg-purple-900/30 border border-purple-100 dark:border-purple-700 rounded-2xl p-4">
          <p className="text-sm text-purple-600 dark:text-purple-400 font-semibold">下月預測{metricLabel}</p>
          <p className="text-2xl font-black text-purple-700 dark:text-purple-300 mt-1">{fmtM(nextMonthForecast)}</p>
          <p className={`text-xs mt-0.5 ${mom >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
            較上月 {mom >= 0 ? '+' : ''}{mom.toFixed(0)}%
          </p>
        </div>
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-700 rounded-2xl p-4">
          <p className="text-sm text-emerald-600 dark:text-emerald-400 font-semibold">未來 {months} 月預測總計</p>
          <p className="text-2xl font-black text-emerald-700 dark:text-emerald-400 mt-1">{fmtM(totalForecast)}</p>
          <p className="text-xs text-emerald-400 dark:text-emerald-500 mt-0.5">含季節性調整</p>
        </div>
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-700/50 rounded-2xl p-4">
          <p className="text-sm text-amber-600 dark:text-amber-400 font-semibold">歷史資料月數</p>
          <p className="text-2xl font-black text-amber-700 dark:text-amber-400 mt-1">{trendData.length}</p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          <span className="text-base font-bold text-gray-700 dark:text-gray-200">歷史實績 + 預測走勢</span>
          <span className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 px-2 py-0.5 rounded-full">藍色 = 歷史</span>
          <span className="text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 px-2 py-0.5 rounded-full">紫色 = 預測</span>
          <span className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 px-2 py-0.5 rounded-full">灰色區間 = 信賴區間 ±15%</span>
        </div>
          ...
          <table className="w-full text-base">
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-700">
                ...
                <th className="text-right px-4 py-3 font-semibold text-gray-500 dark:text-gray-400">預測{metricLabel}</th>
                <th className="text-right px-4 py-3 font-semibold text-gray-500 dark:text-gray-400">低估（-15%）</th>
                <th className="text-right px-4 py-3 font-semibold text-gray-500 dark:text-gray-400">高估（+15%）</th>
                <th className="text-right px-4 py-3 font-semibold text-gray-500 dark:text-gray-400">月增率</th>
              </tr>
            </thead>
            <tbody>
              {forecast.map((f, i) => {
                ...
                return (
                  <tr key={f.yearMonth} className="border-t border-gray-50 dark:border-gray-700 hover:bg-gray-50/60 dark:hover:bg-gray-700/40">
                    <td className="px-4 py-3 font-semibold text-gray-700 dark:text-gray-200">{f.yearMonth}</td>
                    <td className="px-4 py-3 text-right font-mono font-bold text-purple-700 dark:text-purple-400">{fmtM(val)}</td>
                    <td className="px-4 py-3 text-right font-mono text-gray-400 dark:text-gray-500">{fmtM(f.lower)}</td>
                    <td className="px-4 py-3 text-right font-mono text-gray-400 dark:text-gray-500">{fmtM(f.upper)}</td>
      ...
      <div className="text-sm text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800/40 rounded-xl p-3">
