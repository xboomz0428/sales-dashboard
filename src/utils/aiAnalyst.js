...
export function buildAIPayload({ summary, productData, brandData, channelData, channelTypeData, channelCustomerData, customerData, trendData, performanceData, filters, activeBrands, activeProducts, dataThrough, fullOverview }) {
  ...
  const data = {
    ...
    ...(actB || actP ? { 分析範圍說明: '近 3 年無交易的品牌、產品、客戶與通路已從全部數據中排除，不列入任何分析與建議' } : {}),
    ...(fullOverview ? {
...
⚠️ **年度口徑鐵則**：成長策略是「年度」計畫，分析區間＝**本年度 1 月～最新完整月**，目標終點＝**本年 12/31**，嚴禁使用跨年滾動 12 個月當本期。去年資料只能作為「對比參考」（去年同期、去年全年），不得混入本期累計。所有引擎目標都是「今年剩餘月份內」要完成的金額。
（數據中的「年度成長基準」已備好：本年累計、去年同期累計、去年全年、年內剩餘完整月份數——直接引用，不要自己重算錯。）

### 1. 目標數學（先把帳算清楚，全部用年度成長基準）
年度目標 = 去年全年 × 1.25。已完成 = 本年累計。**年內剩餘缺口 X = 年度目標 − 本年累計**，除以剩餘完整月份數 = 今年每月還要做到 NT$ Y（對比去年同月水準差多少）。把 X、Y 算出來寫死；若依同期成長率推算年底自然落點，也請算出「不作為的預期缺口」。後面引擎目標加總必須 ≥ X 的 120%。

### 2. 成長帳拆解（錢從哪裡漏、從哪裡來——本年 vs 去年同期，全部點名）
以「本年累計 vs 去年同期」口徑逐項點名（近3年無交易的品牌/產品/客戶/通路已從數據排除，不會出現，也不要提及）：
- **流失中**：哪些客戶去年同期有貢獻、今年大幅下滑？各流失多少 NT$？（挽回是最便宜的成長）
- **衰退中**：哪些品牌/分類今年 vs 去年同期在掉？各掉多少？年底前止血值多少 NT$？
- **成長中**：哪些品牌/分類/通路今年在漲？年底前加碼天花板估多少？
- **季節性**：從全月度營收找出「今年剩餘月份」中的旺季與其歷史峰值——今年打平或超越峰值，值多少 NT$？

### 3. 五個成長引擎——年內缺口分配表
| 引擎 | 年底前目標金額 | 依據（引用數據） | 主要動作 | 負責角色 |
五個引擎固定為：A 流失客戶挽回（點名對象）／B 既有客戶擴大（回購頻率×客單，耗材訂閱）／C 旺季衝刺（點名今年剩餘月份中的旺季與目標峰值）／D 成長品項加碼（點名品項與通路）／E 新通路或新品（金額目標即可，細節由通路/產品開發分析承接）。五個金額加總 ≥ 缺口 X 的 120%，且全部是「今年剩餘月份內」可實現的目標。
